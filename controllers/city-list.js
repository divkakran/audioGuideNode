...
exports.getCityList = async function(req , res){
    ...
    if(responseToDeliver.status.errorStatus) {
        ...
        CityList.find({city_active: 1} , async (error , cityList) => {
            ...
            for(let item of response){ console.log(response);
               await Monument.find({monument_city_id:mongoose.Types.ObjectId(item._id) } , (err , data) =>{
                   console.log(data);
                   let _temp = JSON.parse(JSON.stringify(data));
                   let monumentArray = []
                   for(let item of _temp){
